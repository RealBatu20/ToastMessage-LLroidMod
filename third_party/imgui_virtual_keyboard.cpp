...
static void ImImplPrivateVirtualKeyboardResetLabels(const char* keyLabels[ImGuiKey_NamedKey_COUNT])   {
    ...
    // Vendored compatibility patch (kyrocute / Toast Message mod):
    // The original gist was written against an ImGui build whose
    // ImGuiKey_NamedKey_COUNT equalled the label array length. Newer ImGui
    // (pinned here to v1.91.6) added more named keys, so the count only needs
    // to be >= the label array. We copy exactly the labels we have; the
    // destination is zero-initialised, so any extra named-key slots stay null.
    IM_STATIC_ASSERT(ImGuiKey_NamedKey_COUNT >= IM_ARRAYSIZE(KeyLabels));
    IM_ASSERT(keyLabels);
    memcpy(keyLabels,&KeyLabels[0],IM_ARRAYSIZE(KeyLabels)*sizeof(char*));
}
...
//#           define IMIMPL_KEY_LABEL(key)  ((key==ImGuiKey_None)?"":KeyLabels[key-ImGuiKey_NamedKey_BEGIN]):
    ...
    if (phyLayout==KPL_COUNT)    {
        ...
        ImFont* font = ImGui::GetFont(); // vendored: FindGlyphNoFallback is non-const in 1.91.6
